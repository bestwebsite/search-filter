<?php
/**
 * Plugin Name:     Bestwebsite Filter
 * Plugin Slug:     Bestwebsite-Search-Filter
 * Plugin Nick:     Bestwebsite
 * Plugin URI:     https://github.com/bestwebsite/search-filter
 * Description:     Search and Filtering system for Pages, Posts, Categories, Tags and Taxonomies.
 * Author:          Bestwebsite
 * Author URI:      http://bestwebsite.com
 * Text Domain:     Bestwebsite-Search-Filter
 * Version:         1.0
 */
...
*/
if (!defined('Bestwebsite_VERSION_NUM'))
    define('Bestwebsite_VERSION_NUM', '1.0');

if (!defined('Bestwebsite_THEME_DIR'))
    define('Bestwebsite_THEME_DIR', ABSPATH . 'wp-content/themes/' . get_template());

if (!defined('Bestwebsite_PLUGIN_NAME'))
    define('Bestwebsite_PLUGIN_NAME', trim(dirname(plugin_basename(__FILE__)), '/'));

if (!defined('Bestwebsite_PLUGIN_DIR'))
    define('Bestwebsite_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . Bestwebsite_PLUGIN_NAME);

if (!defined('Bestwebsite_PLUGIN_URL'))
    define('Bestwebsite_PLUGIN_URL', WP_PLUGIN_URL . '/' . Bestwebsite_PLUGIN_NAME);

if (!defined('Bestwebsite_BASENAME'))
    define('Bestwebsite_BASENAME', plugin_basename(__FILE__));

if (!defined('Bestwebsite_VERSION_KEY'))
    define('Bestwebsite_VERSION_KEY', 'Bestwebsite_version');
...
add_option(Bestwebsite_VERSION_KEY, Bestwebsite_VERSION_NUM);

/*
* Set up Plugin Globals
*/
if ( ! class_exists( 'Bestwebsite' ) )
{
	class Bestwebsite
	{
		...
		public function __construct()
		{
			...
			add_shortcode('Bestwebsite', array($this, 'shortcode'));
		...
		{
			wp_enqueue_style( 'Bestwebsite', Bestwebsite_PLUGIN_URL . '/style.css', false, 1.0, 'all' );
		}
		public function of_enqueue_admin_ss($hook)
		{
			if( 'toplevel_page_Bestwebsite-settings' == $hook )
			{
				wp_enqueue_style( 'of_syntax_style', Bestwebsite_PLUGIN_URL.'/admin/github.css', false, 1.0, 'all' ); //more highlight styles http://softwaremaniacs.org/media/soft/highlight/test.html
				wp_enqueue_style( 'of_style', Bestwebsite_PLUGIN_URL.'/admin/style.css', false, 1.0, 'all' );
				//wp_enqueue_script( 'of_syntax_script', Bestwebsite_PLUGIN_URL.'/admin/syntax.highlight.min.js' );
			}
		}
		...
		*/
		...
		public function get_search_filter_form($submitlabel, $search_placeholder, $fields, $types, $labels, $hierarchical, $hide_empty, $show_count, $post_types, $order_by, $order_dir, $operators, $all_items_labels, $empty_search_url, $add_search_param, $class)
		{
			...
			$returnvar .= '
				<form action="" method="post" class="Bestwebsite'.$addclass.'">
					<div>';
...
// classes
require_once(Bestwebsite_PLUGIN_DIR."/of-list-table.php");
require_once(Bestwebsite_PLUGIN_DIR."/of-taxonomy-walker.php");

// admin screens & plugin mods
require_once(Bestwebsite_PLUGIN_DIR."/of-admin.php");
